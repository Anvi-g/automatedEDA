...
class MasterAgent:
    ...
    async def _run_user_choice(self):
            self.trace.log_event("system", "SYSTEM", "--- Phase 2: User Confirmation & Split ---")
            sess_id = f"{self.session_id}_user_choice"
            await self.runner.session_service.create_session(session_id=sess_id, app_name="agents", user_id=self.user_id)
            
            # Switch Agent
            self.runner.agent = self.agents["user_choice"]
            
            user_msg_start = types.Content(role="user", parts=[types.Part(text="Start the HITL process: get columns, ask user, then split and save.")])
            user_msg_continue = types.Content(role="user", parts=[types.Part(text="Continue the workflow. The system is waiting for the target and split data to be finalized in memory.")])

            # --- Multi-Turn Looping Implementation ---
            
            iteration = 0
            MAX_ITERATIONS = 5 # Safety limit to prevent infinite loops
            
            # Loop until the target_column is defined in shared memory
            while shared_env.get_state("target_column") is None and iteration < MAX_ITERATIONS:
                iteration += 1
                self.trace.log_event("system", "SYSTEM", f"UserChoice Iteration {iteration}")
                
                # Use the starting message only for the first iteration
                current_message = user_msg_start if iteration == 1 else user_msg_continue
                
                # 1. Run the agent for a full cycle (up to its internal stop condition)
                await self._run_observable_loop(sess_id, current_message)
                
                # 2. Check memory after execution
                if shared_env.get_state("target_column") is None:
                    # If still not set, pause briefly. This is VITAL 
                    # to allow the user to read and respond to the input() prompt 
                    # generated by confirm_experiment_setup.
                    self.trace.log_event("system", "SYSTEM", "Waiting 5s for human input or agent re-engagement...")
                    await asyncio.sleep(5) 
                    
            # --- End Looping ---
            
            if shared_env.get_state("target_column") is None:
                self.trace.log_event("system", "ERROR", "HITL process timed out: Target column was not set after multiple iterations.")
            else:
                self.trace.log_event("system", "SYSTEM", "HITL process successfully completed: Target and Split Data are now set.")
